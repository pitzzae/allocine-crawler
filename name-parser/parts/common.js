 ...
 */
var patterns = {
  season: /(([Ss]|((season|saison)[ .]?))([0-9]{1,2}))[Eex.]{1,2}/gi,
  episode: /(([Eex]|(episode[ .]?))([0-9]{2})(?:[^0-9]|$))/gi,
  ...
  proper: /PROPER/,
  repack: /REPACK/,
  container: /MKV|mkv|avi|mp4|MP4|AVI/,
  language: /(multi|vf([fq2]?)|vo((a|stfr))|eng|fr(ench|e)?)/gi,
  widescreen: /WS/,
  website: /^(\[ ?([^\]]+?) ?\])/,
  ...
  for(key in patterns) {
    if(patterns.hasOwnProperty(key)) {
      ...
      if(types[key] && types[key] === 'boolean') {
        ...
        if(clean && types[key] && types[key] === 'integer') {
          clean = parseInt(clean.match(/[0-9]+/gi)[0], 10);
        }
      }
